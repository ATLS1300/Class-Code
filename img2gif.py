...
try:
    ...
    if decide.lower()!='n':
        ...
        img = img.resize(ast.literal_eval(size), resample=PIL.Image.Resampling.BICUBIC)
    elif decide2.lower()!='n':
        scale =float(input("Enter desired scale value. (Ex: 2 will scale twice the size, 0.5 will half it)  \n"))
        img = img.resize((int(img.size[0]*scale), int(img.size[1]*scale)), resample=PIL.Image.Resampling.BICUBIC)
    # breakpoint()
    if transparency and img.mode=='P':
        img.convert('RGBA', palette=Image.Palette.ADAPTIVE).save(outputName, transparency=255) #convert to filetype and save
    elif transparency and img.mode!='P':
        img.convert('RGBA').save(outputName)#, transparency=transparency) #convert to filetype and save
    else:
        img.convert('RGBA',palette=Image.Palette.ADAPTIVE).save(outputName)
